...
# Section 2: Building the Model(First Level) - GLM
```{r, cache=TRUE}
#1) building the model
glm_basemodel <- glm(is_canceled ~.+ adults*children + adults*babies + adults*country + previous_cancellations*previous_bookings_not_canceled + I(previous_bookings_not_canceled * previous_bookings_not_canceled) + booking_changes*previous_cancellations, data=hotel_train_log, family="binomial")

logistic_model_step <- step(glm_basemodel, direction="backward", trace=0)

#2) making prediction
glm_baseprediction <- predict(logistic_model_step, hotel_test_log, type='response')
...
saveRDS(logistic_model_step, "log_model.rds")
```
