...
# Section 2: Building the Model(First Level) - KNN
```{r, cache=TRUE}
#1) building the model
knn_prediction <- knn(hotel_train_knn_x, hotel_test_knn_x, hotel_train_knn_y, k=3, prob = TRUE)
  #(1) to extract the probability part
knn_probability <- attr(knn_prediction, "prob")
knn_binprediction <- ifelse(knn_prediction >=0.2, 1, 0)


#2) evaluating the model
confusionMatrix(as.factor(knn_prediction), as.factor(hotel_test_knn_y), positive = '1')
```
...
saveRDS(knn_prediction, "knn_model.rds")
```